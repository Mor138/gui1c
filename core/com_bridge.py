...
class COM1CBridge:
    ...
    def create_production_task(self, order_ref, rows: list[dict]) -> dict:
        ...
        try:
            ...
            # В конфигурации каталог технологических операций называется
            # "ТехОперации". Ранее использовалось неверное имя,
            # из-за чего поле оставалось пустым.
            doc.ТехОперация = self.get_ref("ТехОперации", "Работа с восковыми изделиями")
            # Для задания требуется указать рабочий центр. Используем одноименный
            # справочник и элемент "восковка" по умолчанию.
            doc.РабочийЦентр = self.get_ref("РабочиеЦентры", "восковка")
            # Ответственного также устанавливаем на администратора системы.
            ...
            for row in rows:
                try:
                    ...
                    # Исправлены имена справочников на актуальные
                    ...
                    item.РабочийЦентр = self.get_ref("РабочиеЦентры", "восковка")
                    # Ссылаемся на исходный заказ, чтобы заполнить поле "Заказ".
                    item.Заказ = base_doc
                    # Конечная продукция и её вариант дублируют номенклатуру, если
                    # иное не указано во входных данных.
